...
class EmbeddingSpaceEvaluator:
    def __init__(self, args=None, embed_net_path=None, lang_model=None, device='cpu'):
        self.n_pre_poses = 4

        # init embed net
        ckpt = torch.load(embed_net_path, map_location=device)
        n_frames = 34
        word_embeddings = None if lang_model is None else lang_model.word_embedding_weights
        n_words = None if lang_model is None else lang_model.n_words
        wordembed_dim = None if args is None else args.wordembed_dim
        mode = 'pose'
        self.pose_dim = ckpt['pose_dim']
        self.net = EmbeddingNet(args, self.pose_dim, n_frames, n_words, wordembed_dim,
